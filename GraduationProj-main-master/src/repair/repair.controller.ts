...
export class RepairController {
    ...
    @Post(':id')
    async makeRepairReq(@Body('products') products: string[], @Param('id') workshopId: string, @Req() req: Request) {
        const userId = req['user'].id;
        const repair = await this.repairService.makeRepairReq(products, userId, workshopId);
        return repair;
    }

    @Patch(':id') 
    async updateRepiarReq(@Param("id") repairId, @Body("status") status: RepairStatus, @Req() req: Request) {
        const userId = req['user'].id;
